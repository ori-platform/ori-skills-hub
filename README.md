...
Signing-key rotation also requires a positive `expected_identity_revision` in
the JSON body. Callers must use the revision returned by registration or the
most recent identity mutation. A stale revision returns `409` before any key or
audit record is created, so overlapping rotations based on the same identity
snapshot have exactly one winner.
